  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const groupSnap = onSnapshot(groupDoc, (queryGroup) => {
        console.log("asdf");
        setGroup(queryGroup.docs.map((g) => ({ ...g.data(), gid: g.id })));
      });
      ...
      const cateSnap = onSnapshot(cateDoc, (queryCate) => {
        ...
        queryCate.docs.map((c) => {
          ...
          const lessonDoc = collection(db, "categories/" + c.id + "/lessons");
          const lessonSnap = onSnapshot(lessonDoc, (queryLesson) => {
    ...
    <Box width={"100%"}>
      {group && (
        <>
          ...
          {nav == "students" && groupData.gid != "" && groupData.cid != "" && (
            <>
              {category.map((c) => {
          ...
          {nav == "lesson" &&
            (lessonData.lid != "" || lessonData.level_id != null) && (
              <>
                <Lesson
                  lessonData={lessonData}
                  lesson={lesson}
